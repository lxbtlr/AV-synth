...
void setup() {
  ...
  style = new int[2 * COUNT]; // color, render stylegyugfvc 
...
void draw() {
  ...
  for (int i = 0; i < COUNT; i++) {
    ...
    if (style[i*2+1] == 0) {
      ...
      noFill();
      strokeWeight(1);
      arcLine(0, 0, pt[index++], pt[index++]+ spectrum[0]*50, pt[index++]);

    } else if(style[i*2+1] == 1) {
      fill(style[i*2]);
      noStroke();
      arcLineBars(0, 0, pt[index++], pt[index++] + spectrum[2]*500, pt[index++]);

    } else {
      fill(style[i*2]);
      noStroke();
      arc(0, 0, pt[index++], pt[index++] + spectrum[5]*1000, pt[index++]);
    }
